...
CountingBloomFilter::CountingBloomFilter(size_t size, uint8_t nHashFunctions)
    : filter(ceil(size / 2.0)), f_set{murmurHash64A_Vector(nHashFunctions)} {}

void CountingBloomFilter::add(const uint8_t *key, uint16_t keyLength) {
...
bool CountingBloomFilter::lookup(const uint8_t *key, uint16_t keyLength) {
    for (size_t i = 0; i < f_set.size(); ++i) {
        size_t index = f_set.at(i)(key, keyLength) % filter.size();
        if (!getNibble(index)) {
            return false;
...
bool CountingBloomFilter::remove(const uint8_t *key, uint16_t keyLength) {
...
uint8_t CountingBloomFilter::getNibble(size_t index) {
    ...
    return (index & 1) == 0 ? filter.at(pos) & LS_NIBBLE_MASK
                            : (filter.at(pos) & MS_NIBBLE_MASK) >> 4;
}

...
void btree_insert_splitted_node(std::vector<uint8_t> promKey, BTreeNode *&left,
                                BTreeNode *&right, uint8_t *key,
                                uint16_t keyLength, uint8_t *value,
                                uint16_t valueLength);

BTreeNode *btree_insert_helper(std::stack<parent> &traverse, BTreeNode *node,
                               uint8_t *key, uint16_t keyLength, uint8_t *value,
                               uint16_t valueLength);

BTreeNode *backtrack(std::stack<parent> &traverse, BTreeNode *node,
                     uint8_t *key, uint16_t keyLength, uint8_t *value,
                     uint16_t valueLength);
...
std::pair<BTreeNode *, int> signpost(BTreeNode *node, uint8_t *key,
                                     uint16_t keyLength);
...
void btree_scan_helper(BTreeNode *node, uint8_t *key, unsigned keyLength,
                       uint8_t *keyOut,
                       const std::function<bool(unsigned int, uint8_t *,
                                                unsigned int)> &found_callback);

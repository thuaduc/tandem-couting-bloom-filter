...
struct Tester {
    ...
    void scan(std::vector<uint8_t> &key,
        ...
        btree_scan(btree, key.data(), key.size(), keyOut,
                   [&](unsigned keyLen, uint8_t *payload, unsigned payloadLen) {
                       ...
                       if (keyLen)
                           assert(memcmp(std_iterator->first.data(), keyOut,
                                         keyLen) == 0);
                       assert(std_iterator->second.size() == payloadLen);
                       if (payloadLen)
                           assert(memcmp(std_iterator->second.data(), payload,
                                         payloadLen) == 0);
                       shouldContinue =
                           found_record_cb(keyLen, payload, payloadLen);
                       ++std_iterator;
